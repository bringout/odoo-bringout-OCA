...
class ReportAction(models.Model):
    ...
    def _compute_display_name(self):
        for report in self:
            ...
            #self.name = self.get_csv_name()
            report.name = report.csv_name or "csv_report"
        return self
    
    csv_name = fields.Char(
        help="CSV name"
    )
    ...
    def _render_csv(self, report_ref, docids, data):
        ...
        if not res_id or not report_sudo.attachment or not report_sudo.attachment_use:
            model_with_context = report_model.with_context(
                **{
                    "active_model": report_sudo.model,
                    "encoding": self.encoding,
                    "encode_error_handling": self.encode_error_handling,
                })
            return model_with_context.create_csv_report(docids, data)
